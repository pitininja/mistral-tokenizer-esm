...
function compressTiktokenBpe(tiktoken_bpe_file: string) {
    ...
    >((memo, item) => {
        if (memo.length === 0) {
            return [{ offset: item[1], tokens: [item[0]] }];
        }
        ...
        memo.push({ offset: item[1], tokens: [item[0]] });
...
(async () => {
    ...
    const maxVocab =
        jsonData.config.default_vocab_size -
        jsonData.config.default_num_special_tokens;
    const slicedVocab = jsonData.vocab.slice(0, maxVocab);
    const simulatedTiktokenFile = slicedVocab
        .map((tokenInfo) => `${tokenInfo.token_bytes} ${tokenInfo.rank}`)
        .join('\n');
