...
export type TokenizerVersion =
    (typeof TokenizerVersion)[keyof typeof TokenizerVersion];
...
export function isValidTokenizerFunction(
    version: unknown
): version is TokenizerVersion {
    return Object.values(TokenizerVersion).includes(
        version as TokenizerVersion
    );
}
...
export type MistralModelAlias =
    (typeof MistralModelAlias)[keyof typeof MistralModelAlias];

function isModelAlias(model: unknown): model is MistralModelAlias {
    return Object.values(MistralModelAlias).includes(
        model as MistralModelAlias
    );
}
...
const aliasToModelMap: Record<MistralModelAlias, MistralModel> = {
    ...
    [MistralModelAlias.OPEN_CODESTRAL_MAMBA]:
        MistralModel.OPEN_CODESTRAL_MAMBA_V01,
...
export function getTokenizerVersionForModel(
    modelOrAlias: MistralModel | MistralModelAlias
): TokenizerVersion {
    const model = isModelAlias(modelOrAlias)
        ? aliasToModelMap[modelOrAlias]
        : modelOrAlias;
...
export function shouldUseTekkenForModel(
    modelOrAlias: MistralModel | MistralModelAlias
): boolean {
    const model = isModelAlias(modelOrAlias)
        ? aliasToModelMap[modelOrAlias]
