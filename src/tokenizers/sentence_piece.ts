...
import { SpecialTokens } from '../commons.js';
...
import { type TokenId, Tokenizer } from './base.js';
...
export class SentencePieceBPETokenizer extends Tokenizer {
    ...
    constructor(tokenizerFilePath: string) {
        ...
        const vocabularyData = readFileSync(
            path.resolve(tokenizerFilePath, 'vocab.bin'),
            'utf-8'
        );
        ...
        const mergesData = readFileSync(
            path.resolve(tokenizerFilePath, 'merges.bin'),
            'utf-8'
        );
    ...
    #decodeVocabulary(binary: string) {
        const byteArray = Uint8Array.from(decodeBase64(binary), (c) =>
            c.charCodeAt(0)
        );
    ...
    get bosId() {
        return this.#vocabByString.get(SpecialTokens.BOS) ?? 0;
    }

    get eosId() {
        return this.#vocabByString.get(SpecialTokens.EOS) ?? 0;
    }

    encode(text: string, shouldAddBosToken = true, shouldAddEosToken = false) {
        if (text.length === 0) {
            return [];
        }
        ...
        const addToMergeQueue = (leftNode: TokenNode) => {
            if (leftNode.next) {
                ...
                const mergeLocation = this.#getMergeByIdentifierString(
                    mergeIdentifierString
                );

                // If mergeLocation not found in merges, that means this merge is not possible according to vocabulary.
                if (mergeLocation) {
                    const mergePrio =
                        mergeLocation + leftNode.origPos / text.length;
        ...
        while (!mergeQueue.isEmpty()) {
            ...
            if (leftOfMerge.mergeToString) {
                const tokenId = this.#vocabByString.get(
                    leftOfMerge.mergeToString
                );
