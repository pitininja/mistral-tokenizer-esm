...
import {
    ...
    isValidTokenizerFunction
} from '../commons.js';
import { compact, groupBy } from '../utils.js';
...
export class Tekkenizer extends Tokenizer {
    ...
    constructor(tokenizerFilePath: string) {
        ...
        const tokenizerData = <TekkenData>(
            JSON.parse(readFileSync(tokenizerFilePath, 'utf-8'))
        );
        ...
        this.#specialTokens = [
            ...
            ...Array.from(
                {
                    length:
                        default_num_special_tokens - this.#SPECIAL_TOKENS.length
                },
                (_, i) => `<SPECIAL_${i}>`
            )
        ];
        ...
        this.vocab = Array.from({ length: default_vocab_size }, (_, i) =>
            this.#idToPiece(i)
        );
    }
    ...
    #decodeAll(
        tokens: ReadonlyArray<TokenId>,
        tokenPolicy: SpecialTokenPolicy
    ) {
        const decoded: Array<string> = [];

        const groupedTokens = groupBy(tokens, (t) =>
            String(t < this.#numSpecialTokens)
        );
        ...
        for (const [key, group] of groupedTokens.entries()) {
            ...
            if (isSpecial) {
                ...
                if (tokenPolicy === SpecialTokenPolicy.KEEP) {
                    decoded.push(...group.map((t) => this.#specialTokens[t]));
                }
            } else {
                decoded.push(
                    this.#model.decode(
                        group.map((t) => t - this.#specialTokens.length)
                    )
                );
            }
        }
    ...
    decode(tokenIds: ReadonlyArray<TokenId>) {
        ...
        return this.#decodeAll(
            hasBosToken ? tokenIds.slice(1) : tokenIds,
